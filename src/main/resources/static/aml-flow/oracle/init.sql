...
--创建视图-基于base_表
CREATE  OR  REPLACE  VIEW  ACT_ID_GROUP AS   SELECT 	CODE as ID_, null as REV_ ,NAME as NAME_, 'role' AS TYPE_ FROM BASE_GROUP;

CREATE  OR  REPLACE  VIEW  ACT_ID_INFO AS   SELECT null as ID_,null as REV_,null as USER_ID_,null as TYPE_,null as KEY_,null as VALUE_,null as PASSWORD_,null as PARENT_ID_ from dual;

CREATE  OR  REPLACE  VIEW  ACT_ID_MEMBERSHIP AS  SELECT DISTINCT
	temp."USER_ID_",
	temp."GROUP_ID_" 
FROM
	(
	SELECT
		...
		BG.CODE AS GROUP_ID_ 
	FROM
		BASE_GROUP_MEMBER BGM
		LEFT JOIN BASE_GROUP BG ON BGM.GROUP_ID = BG.ID
		LEFT JOIN BASE_USER BU ON BGM.USER_ID = BU.ID UNION
	SELECT
		BU.USERNAME AS USER_ID_,
		BG.CODE AS GROUP_ID_ 
	FROM
		...
		LEFT JOIN BASE_GROUP_RELATION bgr ON bgr.TYPE_ID = bgm.GROUP_ID
		LEFT JOIN BASE_GROUP BG ON bgr.GROUP_ID = BG.ID 
	) temp 
WHERE
	temp.USER_ID_ IS NOT NULL 
	AND temp.GROUP_ID_ IS NOT NULL;

CREATE  OR  REPLACE  VIEW  ACT_ID_USER AS   SELECT username as ID_ ,null as REV_,NAME AS FIRST_,null as LAST_ , EMAIL AS EMAIL_ ,NULL AS PWD_ ,NULL AS PICTURE_ID_ FROM BASE_USER;